...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      apiClient.get("/api/auth/me").then(userResponse => {
        saveSession(token, userResponse.user);
        router.push("/dashboard");
      }).catch(() => {
        // If can't get user, just redirect to login
        router.push("/login");
      });
    }
  }, [router, searchParams]);
    ...
    <main className="container auth-wrap">
      ...
      <section className="glass-card auth-card fade-up">
        <div className="card-pad">
          ...
          <p className="subtitle">Use your account to run startup analysis workflows.</p>
